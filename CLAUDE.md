...
| **`docker system prune` deletes stopped containers**, and `codespace down` stops rather than removes — so an idle codespace vanished, showing as `stopped` → `absent` in `codespace list`. `--filter until=168h` does *not* fix this: `until` matches container **creation** time, not last use, so a codespace used daily was reaped for merely being a month old | Exempt them from the prune entirely — `autoPrune.flags = [ "--filter" "until=168h" "--filter" "label!=codespace" ]`, matching the launcher's `--id-label` |
