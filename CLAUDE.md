...
| **`--remote-env` and `--secrets-file` do not persist.** They apply to that invocation's user commands, not to the container environment — but `--secrets-file` *does* reach them, including the dotfiles `install.sh`, and takes a path so no value lands on a command line | Use both: `--secrets-file` for create time, and a `0600` `~/.codespace-secrets` sourced from `/etc/profile.d` for every shell after. The editor inherits it by being started from a login shell, so nothing needs `docker exec -e` |
...
| **`pgrep -f` inside `sh -c` matches itself**, because the pattern is in the shell's own command line. Bracketing the first character is *not* enough: a pattern like `code-server.*--bind-addr` still matches when some other part of the same command line supplies `code-server` (e.g. `/tmp/code-server.args`) and `.*` bridges to the bracketed pattern text | Do not infer "is it running" from a process listing at all. Ask the thing itself — an HTTP probe of the endpoint — which is also the property actually worth knowing |
| **`docker system prune` deletes stopped containers**, and `codespace down` stops rather than removes — so an idle codespace vanished, showing as `stopped` → `absent` in `codespace list`. `--filter until=168h` does *not* fix this: `until` matches container **creation** time, not last use, so a codespace used daily was reaped for merely being a month old | Exempt them from the prune entirely — `autoPrune.flags = [ "--filter" "until=168h" "--filter" "label!=codespace" ]`, matching the launcher's `--id-label`. Retention is a separate concern with a real clock: `codespace gc`, on a daily timer, drops containers idle past `RETENTION_DAYS` (default 30, as in real Codespaces). Last use is recorded by the launcher, never inferred — `.Created` is the wrong event and `.State.FinishedAt` is bumped by every VM shutdown |
| **The code-server installer caches into `$HOME/.cache`** — container layer, so every new codespace re-downloaded the same ~200 MB release tarball. Its `fetch()` does reuse an existing file; the cache just never survived | Bind-mount `/persist/codespace/cache` into every container and run the installer with `XDG_CACHE_HOME` pointing at it, so a version downloads once and later containers only untar. `sudo env VAR=…`, not `sudo VAR=…`: the bare form depends on the container's sudoers env policy, which is not ours to assume across base images |
...
| **Docker's named-volume namespace is flat and global to the daemon.** Two `devcontainer.json` files naming the same `source=` get the *same* volume, mounted into both containers at once, with no coordination and no warning. Copying a config into a second repo is all it takes | `${devcontainerId}` in the volume name for anything holding real state — it is unique per dev container on a host and stable across rebuilds, so persistence survives but collision cannot happen. A fixed name only where sharing is the intent, as with the Nix store, and then said so in a comment |
...
| **The VM kernel has no legacy iptables tables** (6.18, `CONFIG_NETFILTER_XTABLES_LEGACY` unset), while the `docker-in-docker` feature switches to `iptables-legacy` on every Debian/Ubuntu except `resolute`. A container borrows the kernel, so the inner `dockerd` dies on its first chain, the init script gives up after five tries, and `up` still reports success | The launcher (`fix_dind_iptables`) points `iptables`/`ip6tables` at the `-nft` binaries (same syntax, nftables backend) and restarts `dockerd` through the feature's own `docker-init.sh`, after waiting out the entrypoint's retry loop by the container's start time. Success is `docker info` answering, never a process listing. Launcher, not `postCreateCommand`: real Codespaces still has the legacy tables |
| **The `docker-in-docker` entrypoint mounts a tmpfs over `/tmp`** whenever `/tmp` is not already a mountpoint, hiding anything bind-mounted beneath it — the agent socket at `/tmp/ssh-agent.sock` vanished while `docker inspect` still listed it, so the launcher configured agent mode against nothing | The socket lives at `/mnt/git-ssh/agent.sock`, beside the rest of git's credentials. `/mnt` is claimed by no image or feature. Plumbing goes in `/mnt`; user-facing paths such as `/workspaces/refs` stay where Codespaces puts them. Containers created before the move keep the old path until rebuilt, and the launcher reads which one a container actually has |
